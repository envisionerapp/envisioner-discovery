...
router.post('/validate', async (req, res) => {
  try {
    ...
    // 3. Check if user exists in main users table (envisioner app users)
    const mainUser = await db.$queryRaw<{ email: string }[]>`
      SELECT email FROM users WHERE LOWER(email) = ${normalizedEmail}
      LIMIT 1
    `;
